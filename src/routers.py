...
from service.swarm_tokens_logging import count_tokens
...
async def generate_architecture(request: ArchitectureRequest) -> ArchitectureResponse:
    ...
    try:
        ...
        deployment_response = get_agent_response(
            agent=deployment_agent, messages=[user_message]
        )
        testing_response = get_agent_response(
            agent=testing_agent, messages=[user_message]
        )
        research_response = get_agent_response(
            agent=research_agent, messages=[user_message]
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate architecture: {str(e)}"
        )
